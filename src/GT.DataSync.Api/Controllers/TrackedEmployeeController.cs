...
namespace GT.DataSync.Api.Controllers
{
    ...
    {
        private readonly TrackedSampleDataService _employees;

        public TrackedEmployeeController(TrackedSampleDataService employees)
        {
        ...
        public Task<EmployeeSyncResponse> GetPagedWithReconcile([FromQuery] string? token)
        {
            ...
            return Task.FromResult(new EmployeeSyncResponse
            {
                Control = new()
                {
                    ...
                    NoMoreData = data.Count < 5 ? true : null,
                    TotalItems = _employees.GetEmployees().Count(x => x.Delete != true)
                },
                Employees = data
            });
        }
